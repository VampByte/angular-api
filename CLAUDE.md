...
**IMPORTANTE — errores con status 200:** esta API nunca devuelve un status HTTP de error (4xx/5xx). Un id inválido en `GET/:id`, `PUT/:id` o `DELETE/:id` responde `200 OK` con `{"error": "mensaje"}` en el body. `HttpClient` no rechaza la promesa en ese caso (solo lo hace en status no-2xx), así que un `try/catch` alrededor de la llamada HTTP directa **no detecta el error**. `UsersService` maneja esto centralizado en el helper privado `assertOk()`, que revisa si la respuesta tiene una propiedad `error` y la convierte en una excepción real. Cualquier método nuevo del servicio que pegue contra `/:id` debe pasar la respuesta por `assertOk()` — si no, el error queda silencioso y el componente que llama recibe un objeto `{error: "..."}` disfrazado de `IUser`.
